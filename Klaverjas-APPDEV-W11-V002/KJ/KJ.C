...
float fact[19]={1.0,1.0,2.0,6.0,24.0,120.0,720.0,5040.0,40320.0,362880.0,
...
L4 { tnoord[n] = 1;   /* gedraaide kaarten al gedraaid ? */ 
 ...
 SLAG=0;
 if(COMP) troef_bepalen(); 
  else {
	  if(startvrager==2) troef_bepalen();
	  else
	  {
 ...
 for(SLAG=1;SLAG<9;SLAG++)
  {
   TACTIEK=0;
   speler1();
   if(!legkaart(Lkaart,Lkleur,VRAGER) || !check_valid()){error_legkaart(); break;}
   startvrager=VRAGER;
   if(TACTIEK==41) TACTIEK41=TRUE;
   tac[TACTIEK]++;
/*if(TACTIEK==0){error_legkaart(); break;}*/
   TACTIEK=0;
   tegenspeler1();
   if(!legkaart(Lkaart,Lkleur,VRAGER) || !check_valid()) {error_legkaart(); break;}
   tac[TACTIEK]++;
	   ...
	   while(TRUE)
	      {
	       while(!number_presses_down(0))
		    if(bioskey(1))   {n=getch();  if(n==27) einde();  break;}
          if(n==100)       {DEMO=TRUE;  DICHT=0; } /* als n='d' Bekijk alle kaarten*/
          if(n==101)       {DEMO=FALSE; DICHT=1;}  /* als n='e' en zet het weer uit*/
	       if(n) break;
	       while(!number_presses_up(0));
	      }
	   }
     else {  setcolor(WHITE);  outtextxy(1,10*10,"Druk toets ");	 getch();  }
    }
   if(SLAG<8)
   {
   setcolor(BLACK);
...
if(COMP==TRUE)
{
 for(n=0;n<76;n++)
  ...
  printf("Nat      %8ld | %8ld\n",Nat[1],Nat[0]);
  printf("Superroem %8ld",Superroem);
  getch();
 exit(1);
 }
...
float kans_hogerr,kans_kaartt,kans_troefkaartt;
...
/*	gotoxy(20,11);
	...
	gotoxy(20,13);
	printf("%d",VRAGER);*/
	if(kans_kaartt < 0.6 && strlen(Krt_dicht[TROEF]))                  /* 0.4 naar 0.6 veranderd jan 2005 */
	  {
...
/* Als ik troef op tafel en hij kale A of tien van kleur ik niet
 ...
 if(!Skaart) {      				/* Zoek roemkans & slagkans>0.75 Tafel*/
 Skleur=5;
  m=j=0;
  i=1;
  L4 {if(tafel[0][n].slagkans>85 )         /* slagkans 75 -> 85 jan 2005 */
       {
	...
	if(i>j)
	{
	 ...
	 TACTIEK=10;
   										...
   										/* Zoek roemkans & slagkans>0.75 Hand*/
  L8 {if(hand[0][n].slagkans> 85 )                                 /* slagkans 75 -> 85 jan 2005 */
       {
...
/* gegarandeerde slagen en troef trekken als ts nog misschien troef */
 ...
 if(!Skaart) {
 L4 { if(tafel[0][n].gegarandeerd==1 ) /* gegarandeerde slagen tafel uitspelen*/
     {
	 Skaart = tafel[0][n].naam;
	 Skleur = tafel[0][n].kleur;
	 TACTIEK=55;
 ...
 if(!Skaart) {
  L8 {if(hand[0][n].gegarandeerd==1)   /* gegarandeerde slagen hand uitspelen*/
       {
	 Skaart = hand[0][n].naam;
	 Skleur = hand[0][n].kleur;
	 TACTIEK=56;
...
if(!Skaart)   {
    L32	{ if(kaart[n].DichtIkHy==VRAGER && kaart[n].kleur==Skleur) 
	    {
	     ii=i=wie_slag();
	     if(ii>2) ii-=2;
	     j=VRAGER;
		 ...
		 {          TACTIEK=43;    Skaart=hoogsteroem(Skleur);}
	       else        {TACTIEK=32;    Skaart=laagsteroem(Skleur);}
}
	    }}}
 ...
 if(ii==j) /* Als speler slag heeft */
   {
    for(m=0;m<SLAGKRTNO;m++) /* als slagkans >0.75*/
      if(slag[SLAG][m].speler==i && slag[SLAG][m].kans>75)
	 {
...
if(!Skaart)   {       /* geen idee wat hij doet */
    m=99;
