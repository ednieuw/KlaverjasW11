...
internal static class Program
{
    private static int Main(string[] args)
    {
        ...
        if (args.Length > 0 && args[0] == "ble") return BleTest.Uitvoeren();
        ...
        Console.WriteLine($"Superroem Zuid/Noord: {s.Superroem[0]} / {s.Superroem[1]}");
    ...
    /// <summary>
    /// Laat de tactiek van Ednieuw en de zoekende speler van Ronlog zonder scherm
    ...
    {
        int gevraagd = args.Length > 1 && int.TryParse(args[1], out var a) ? a : 500;
        // Elk spel wordt twee keer gespeeld, dus de helft van de grens van een
        // miljoen: daarboven lopen de 32-bits tellers over.
        int spellen = Math.Clamp(gevraagd, 1, 500_000);
        if (spellen != gevraagd)
            Console.WriteLine($"teruggebracht tot {spellen} spellen: daarboven lopen de tellers over");
        int zaad = args.Length > 2 && int.TryParse(args[2], out var b) ? b : 1;

        Console.WriteLine($"Ednieuw (vuistregels) tegen Ronlog (zoekend): {spellen} spellen, elk twee keer gespeeld");
        Console.WriteLine();

        long[] punten = new long[2];      // 0 = Ednieuw, 1 = Ronlog
        ...
        {
            // ronde 0: Ednieuw is Zuid. ronde 1: Ronlog is Zuid, zelfde kaarten.
            bool ednieuwIsZuid = ronde == 0;
            var spel = new KjSpel(new StilleUi(), zaad);
            var S = spel.E.S;
            var E = spel.E;
            S.Comp = true;
            S.Zoekt[0] = !ednieuwIsZuid;       // Zuid
            S.Zoekt[1] = ednieuwIsZuid;        // Noord
            ...
            for (int i = 0; i < spellen; i++)
            {
                ...
                int ed = ednieuwIsZuid ? 0 : 1;
                int lo = ednieuwIsZuid ? 1 : 0;
        ...
        Console.WriteLine($"{"",-22}{"Ednieuw",10}{"Ronlog",10}");
        ...
        if (tot > 0)
            Console.WriteLine($"Puntenaandeel      : Ednieuw {100.0 * punten[0] / tot:F1}%   Ronlog {100.0 * punten[1] / tot:F1}%");
        long totG = gewonnen[0] + gewonnen[1];
        if (totG > 0)
            Console.WriteLine($"Spellen gewonnen   : Ednieuw {100.0 * gewonnen[0] / totG:F1}%   Ronlog {100.0 * gewonnen[1] / totG:F1}%");
