...
public partial class KjEngine
{
    ...
    /// <summary>
    /// Door <see cref="EvalueerSpel"/> gezet zodra een hele partij (1500 punten)
    /// net is afgerond, voor de UI om er een duidelijke, feestelijke melding
    /// van te maken — <c>S.PuntenTotaalSpel</c> zelf staat alweer op nul tegen
    /// de tijd dat de aanroeper terug is.
    /// </summary>
    public bool PartijNetUit { get; private set; }
    public int PartijEindstandZuid { get; private set; }
    public int PartijEindstandNoord { get; private set; }
    ...
    public void Delen()
    {
        ...
        for (int m = 31; m >= 0; m--)
        {
            // S.Random(m + 1) en niet S.Random(m). Een eerlijke Fisher-Yates
            // kiest uit alle nog niet gedeelde kaarten, dus 0..m. KJJ.C schreef
            // random(m) - 0..m-1 bij Borland - waardoor de kaart die op dat
            // moment op plek m lag bij die stap nooit gekozen kon worden.
            //
            // Dat is niet onschuldig: over 200.000 keer delen kreeg Zuid 60,34
            // kaartpunten per spel en Noord 59,66, met afwijkingen tot 10% voor
            // afzonderlijke kaarten. Alles zat in de tafel- en dichte stapels;
            // de handen waren wel gelijk.
    ...
    public void TroefBepalen()
    {
        ...
        // Zekere slagen tellen mee bij de troefkeuze. Ze horen uit de eigen
        // kaarten te komen, en die staan in Hand[0] en Tafel[0]: Vulhanden()
        // vult index 0 met "mijn kant" en index 1 met de tegenpartij, niet Zuid
        // en Noord.
        //
        // KJJ.C regel 729-733 schrijft daar hand[VRAGER-1] en tafel[VRAGER-1].
        // Dat klopt alleen als VRAGER 1 is. Maakt Noord troef (VRAGER 2), dan
        // wordt index 1 gelezen - en Gegarandeerd wordt daar nooit berekend en
        // in de wislus hierboven ook niet teruggezet. Noord koos zijn troef dus
        // mede op een blijven-hangen vlag uit een eerdere stand. Meetbaar:
        // Noord koos harten ruim 16% vaker dan Zuid, en hield er over de hele
        // partij ongeveer 2,4 punt per spel aan over.
        for (n = 0; n < 8; n++)
            if (S.Tafel[0, n].Gegarandeerd != 0)
                zekereslagen[S.Tafel[0, n].Kleur]++;

        for (n = 0; n < 8; n++)
            if (S.Hand[0, n].Gegarandeerd != 0)
                zekereslagen[S.Hand[0, n].Kleur]++;
    ...
    public SlagUitslag Evalueer()
    {
        ...
        {
            roem += (n0 == 'B') ? 200 : 100;
            // Vier gelijke kaarten gaan naar wie de slag pakt - dezelfde kant
            // die de roem krijgt. Zo is te zien waar ze vielen.
            S.Superroem[S.StartVrager - 1]++;
        }
    ...
    public string EvalueerSpel()
    {
        ...
        if (S.PuntenTotaalSpel[0] >= 1500 || S.PuntenTotaalSpel[1] >= 1500)
        {
            ...
            PartijNetUit = true;
            PartijEindstandZuid = (int)S.PuntenTotaalSpel[0];
            PartijEindstandNoord = (int)S.PuntenTotaalSpel[1];
