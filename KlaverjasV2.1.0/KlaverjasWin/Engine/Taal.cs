...
public static class Taal
{
    ...
    public static string MenuSamenSpelen => T("&Samen spelen (zoeken)", "&Play together (search)");
    ...
    /// <summary>Grote, feestelijke titel zodra een hele partij is gewonnen.</summary>
    public static string PartijGewonnen(bool zuidWon)
        => T($"{(zuidWon ? "Zuid" : "Noord")} wint de partij!",
             $"{(zuidWon ? "South" : "North")} wins the match!");

    public static string PartijGelijkspel => T("Gelijkspel!", "Tie game!");

    public static string PartijEindscore(int zuid, int noord) => $"{Zuid} {zuid}  –  {Noord} {noord}";
    ...
    // ---------------------------------------------------- samen spelen (BLE)
    public static string GastZoekt =>
        T("Zoeken naar een tegenstander op Mac of iPhone...",
          "Searching for an opponent on Mac or iPhone...");
    public static string GastVerbindtMet(string naam) =>
        T($"Verbinden met {(string.IsNullOrEmpty(naam) ? "apparaat" : naam)}...",
          $"Connecting to {(string.IsNullOrEmpty(naam) ? "device" : naam)}...");
    public static string GastWachtOpBegroeting =>
        T("Verbonden, wachten op begroeting...", "Connected, waiting for greeting...");
    public static string GastVerbonden(string naam) =>
        T($"Verbonden met {naam}", $"Connected to {naam}");
    public static string GastVerbroken =>
        T("Verbinding verbroken. Kies Samen spelen opnieuw om het nogmaals te proberen.",
          "Connection lost. Choose Play together again to retry.");
    public static string GastFout(string tekst) =>
        T($"Fout bij samen spelen: {tekst}", $"Error while playing together: {tekst}");
    public static string GastVorigePartijReageertNiet =>
        T("De lopende partij reageert niet; probeer het zo nog eens.",
          "The current game is not responding; please try again shortly.");
