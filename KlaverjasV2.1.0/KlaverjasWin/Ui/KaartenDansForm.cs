...
public sealed class KaartenDansForm : Form
{
    ...
    private readonly System.Windows.Forms.Timer _klok = new() { Interval = 20 };
    ...
    /// <summary>Twee bewegingsstappen per tik, elke 20 ms — half zo snel als voorheen.</summary>
    private void Stap()
    {
        ...
        for (int herhaling = 0; herhaling < 2; herhaling++)
        {
            ...
            // In het origineel breekt dit de kaart af en begint de buitenste
            // lus met een nieuwe, willekeurige kaart: zodra de horizontale
            // snelheid èn de positie beide bijna nul zijn. Dit stond er eerder
            // niet in — zonder deze regel bleef één kaart de hele dans lang
            // over het hele scherm stuiteren (ook onderin) in plaats van dat
            // er, zoals in het origineel, steeds opnieuw een kaart van links
            // over het bovenste deel van het scherm kwam aanvliegen.
            if (Math.Abs(_x) < 4 && Math.Abs(_posx) < 5) { NieuweKaart(); continue; }
            ...
            if (_posx < 1)
            {
                _posx = 0;
                if (_x > -2) _x = -3;
                _x = (-_x * 7) / 10;   // was per ongeluk (-_x*3)/4
                _posx += _x / 2;
            }
            if (_posy > _virtH - 85)
            {
