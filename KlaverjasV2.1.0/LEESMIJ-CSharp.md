...
| `KlaverjasWin/Ble/` | de gast-kant van het samenspel-protocol (bluetooth), zie hieronder |
| `KlaverjasTest/` | testharnas dat de engine tegen zichzelf laat spelen, en `ble` toetst de Ble-laag |
...
Dezelfde regels, met daaronder hoe vaak elke tactiek is toegepast. Op één punt wijkt de
volgorde af: de stand van de partij staat bovenaan, want daar kijk je tijdens het spelen
naar.

Het venster staat naast het spel in plaats van ervoor, en werkt zichzelf twee keer per
seconde bij — bij snel spelen is dat het enige dat er nog leeft, en het gaat daarom vanzelf
open zodra snel spelen aangaat. De rijen worden overschreven in plaats van opnieuw
opgebouwd; anders knippert de lijst twee keer per seconde.

De tellingen blijven bewaard in `%AppData%\Klaverjas\klaverjas.json`, samen met de twee
speelwijzen en de taal — zie `Engine/Bewaarplaats.cs`. Bewust níet bewaard: demo, open
kaart en snel spelen; die horen bij één zitting. De knop **Wissen** zet alleen de tellingen
op nul. De volgorde luistert daarbij nauw: eerst de motor stil, dan de momentopname leeg,
en pas dan wissen — het stilzetten legt de stand namelijk nog één keer vast, en zou de
zojuist gewiste tellingen anders meteen terugschrijven.

`Tactieknamen.cs` is ooit automatisch gegenereerd en blijft daarom ongemoeid; tactiek 70
...
bestanden in `Ui/` zijn Windows-gebonden. `Engine/Toevalsreeks.cs` is een generator van
vier regels die in elke taal dezelfde reeks oplevert; daardoor is een omzetting naar een
andere taal te *bewijzen* in plaats van te hopen.
...
(klaver, schoppen, ruiten, harten). `spoor-v11.txt` in deze map is het ijkbestand: 200
spellen met zaad 1, 6601 regels, van versie 1.1. Komt een nieuw spoor daar regel voor
regel mee overeen, dan is er niets aan het spelverloop verschoven.

Het oudere `spoor-csharp.txt` hoorde bij versie 1.0 en is niet meer na te spelen: de twee
reparaties hieronder veranderen het spelverloop bij elk spel.
...
### Eén bewuste afwijking van 1994: de superroem

Op één punt rekent deze versie anders dan het origineel: **vier gelijke kaarten in één
slag leveren nu 100 roempunten op, en 200 bij vier boeren.**
...
gebeurde het twaalf keer. In de 200 spellen van het ijkbestand komt het niet voor, dus het
spoor alleen legt dit gedrag niet vast. De teller staat sinds versie 1.1 per kant in het
statistiekenscherm, zodat je kunt zien waar het viel.
...
## Samen spelen via bluetooth

`KlaverjasWin/Ble/` is de Windows-kant van het samenspel-protocol waarmee deze app en de
Swift-app op Mac/iPhone/iPad tegen elkaar kunnen spelen, zonder dat de twee motoren
tijdens zo'n partij gelijk hoeven te lopen — er draait er dan maar één, bij de kant die
zich openstelt. Volledig uitgewerkt in `BLUETOOTH-VOOR-WINDOWS2.1.0.md`; hier alleen waar wat
staat.

| Bestand | Inhoud |
|---|---|
| `Ble/UartDienst.cs` | de drie vaste bluetooth-UUID's |
| `Ble/RegelBuffer.cs` | een regel opknippen voor verzending, en binnenkomende pakketjes weer aan elkaar plakken tot regels |
| `Ble/DuoBericht.cs` | de zes regelvormen (`KJ`/`STAT`/`JA`/`STAND`/`ZET`/`P`) parsen en samenstellen |
| `Ble/TekenJsonConverter.cs` | een kaartnaam is in de JSON een object (`{"raw": 65}`), geen kaal getal of tekst |
| `Ble/SpelStandGast.cs` | de inkomende `STAND` (`SpelStandGast`) en de uitgaande `GastZet` |
| `Ble/DuoStand.cs` | `DeflateStream` + base64 eromheen — let op: dit is rauwe DEFLATE (RFC 1951), geen echte zlib-stream, ook al heet de Mac-kant zijn functie `COMPRESSION_ZLIB` |
| `Ble/GastWeergave.cs` | zet een `SpelStandGast` om in een gewone `SpelView`, met Zuid/Noord omgedraaid zodat de gast zijn eigen kant altijd onderin ziet — de bestaande tekencode in `SpelForm` hoeft daar niets voor te weten |
| `Ble/GastRadio.cs` | de central-rol (`Windows.Devices.Bluetooth`): scannen, verbinden, de begroeting afhandelen, STAND ontvangen, ZET versturen |

**Windows is voorlopig altijd gast, nooit gastheer.** De central-rol (scannen/verbinden)
is een oude, stabiele WinRT-API; zelf adverteren als GATT-server (`GattServiceProvider`)
hangt af van de adapter en is minder betrouwbaar. De Mac/iPhone-kant stelt zich dus altijd
open ("Samen spelen" → openstellen); Windows zoekt via *Spel → Samen spelen (zoeken)*.

**Protocolversie 3: `STAT` erbij.** Vlak na de `KJ`-begroeting wisselen beide kanten een
`STAT <base64>` uit (bewaarde score per partner). Windows houdt zelf nog geen score per
partner bij: `GastRadio` stuurt daarom altijd dezelfde vaste, lege `Statistiek` (`{}`) en
negeert de inhoud van de `STAT` die terugkomt — genoeg om te verbinden en te spelen, zie
"STAT: score per partner" in `BLUETOOTH-VOOR-WINDOWS2.1.0.md`. Dit is ook waarom de
protocolversie van 2 naar 3 ging: twee kanten met een verschillend versienummer weigeren
te beginnen.

**`{"verder": true}`:** de "volgende slag"-tik mag ook van de Windows-kant komen, niet
alleen van de gastheer. Zodra een binnenkomende `STAND` `wachtOpVerder == true` heeft, mag
er (van welke kant dan ook, wie het eerst is) geklikt of een toets ingedrukt worden — net
als bij de lokale `Verder`-wachtstand, alleen gaat er nu een `GastZet.VoorVerder()` over de
lijn in plaats van de lokale wachtsemafoor los te laten.

**Wat wél en niet getoetst is.** De coderingslaag (JSON ↔ deflate ↔ base64, inclusief de
`Teken`-vorm van een kaartnaam) en de regelbuffer zijn getoetst zonder bluetooth, tegen de
kant-en-klare voorbeelden uit `BLUETOOTH-VOOR-WINDOWS2.1.0.md` en tegen losse
byte-arrays — zie `dotnet run --project KlaverjasTest -- ble`. **`GastRadio.cs` zelf is op
deze machine niet tegen een echte gastheer getoetst** (geen bluetooth-hardware/geen tweede
toestel hier beschikbaar): de WinRT-aanroepen zijn zorgvuldig volgens de documentatie
overgenomen, maar "compileert" is niet hetzelfde als "verbindt echt". Test dat met twee
echte toestellen zoals `BLUETOOTH-VOOR-WINDOWS2.1.0.md` voorschrijft: eerst de Mac/iPhone in
demo-stand, dan pas een mens aan de Windows-kant.
...
* Menu *Spel → Samen spelen (zoeken)*: verbindt via bluetooth met een gastheer op Mac of
  iPhone/iPad en speelt tegen die partij mee, zie "Samen spelen via bluetooth" hierboven.
  Sluit uit met snel spelen: allebei nemen ze het scherm en de klikafhandeling over.
...
7. **Vier gelijke kaarten leveren nu wél roem op.** *Dit is de enige plek waar het spelgedrag
   ...
   rekening mee, net zomin als het origineel dat deed.
